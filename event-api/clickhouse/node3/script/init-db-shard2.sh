...
clickhouse client -n <<-EOSQL
  ...
  CREATE TABLE IF NOT EXISTS shard.films_bookmarks(
      ...
      topic String,
      partition UInt64
    ) ENGINE = ReplicatedMergeTree('/clickhouse/tables/shard2/films_bookmarks', 'replica_bookmarks_1')
  ...
  CREATE TABLE IF NOT EXISTS replica.films_bookmarks(
      ...
      topic String,
      partition UInt64
    ) ENGINE = ReplicatedMergeTree('/clickhouse/tables/shard1/films_bookmarks', 'replica_bookmarks_2')
    PARTITION BY toYYYYMMDD(timestamp)
    ORDER BY user_id;
  ...
  CREATE TABLE IF NOT EXISTS shard.films_progress(
      ...
      topic String,
      partition UInt64
    ) ENGINE = ReplicatedMergeTree('/clickhouse/tables/shard2/films_progress', 'replica_progress_1')
    PARTITION BY toYYYYMMDD(timestamp)
    ORDER BY user_id;
  ...
  CREATE TABLE IF NOT EXISTS replica.films_progress(
      ...
      topic String,
      partition UInt64
      ) ENGINE = ReplicatedMergeTree('/clickhouse/tables/shard1/films_progress', 'replica_progress_2')
      PARTITION BY toYYYYMMDD(timestamp)
      ORDER BY user_id;
  ...
  CREATE TABLE IF NOT EXISTS shard.films_feedbacks(
      ...
      topic String,
      partition UInt64
    ) ENGINE = ReplicatedMergeTree('/clickhouse/tables/shard2/films_feedbacks', 'replica_feedback_1')
    PARTITION BY toYYYYMMDD(timestamp)
    ORDER BY user_id;
  ...
  CREATE TABLE IF NOT EXISTS films_feedbacks(
      ...
      topic String,
      partition UInt64
      ) ENGINE = ReplicatedMergeTree('/clickhouse/tables/shard1/films_feedbacks', 'replica_feedback_2')
      PARTITION BY toYYYYMMDD(timestamp)
      ORDER BY user_id;
